...
///
/// # Returns
/// The corresponding word if found, otherwise `None`.
...
/// # Arguments
/// * `word_list` - The loaded BIP-39 wordlist.
/// * `decimal` - The decimal value to convert (must be in the range 0 to 2047).
///
/// # Returns
/// The corresponding word if found, otherwise `None`.
pub fn decimal_to_word(word_list: &WordList, decimal: u32) -> Option<&str> {
    if decimal > 2047 {
        return None; // Ensure input is within the valid range
    }
...
///
/// # Returns
/// The corresponding binary string if found, otherwise `None`.
...
///
/// # Returns
/// The corresponding decimal value if found, otherwise `None`.
